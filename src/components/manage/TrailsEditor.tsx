...
import { setTrailsPublished } from '@/actions/event';
...
export default function TrailsEditor({ published, sections }: { published: boolean; sections: Section[] }) {
  const t = useTranslations('manage');
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  ...
  const addSection = () => {
    startTransition(async () => {
      await createInfoSection('trails');
      router.refresh();
    });
  };
  ...
  const togglePublish = () => {
    startTransition(async () => {
      await setTrailsPublished('en', !published);
      router.refresh();
    });
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-ink-light">{t('trailsHint')}</p>
