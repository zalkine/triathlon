...
export default async function HomePage() {
  ...
  // The home page has two lives. Until the results are published it counts down
      ...
      <main className="flex flex-1 flex-col items-center px-6 py-6 sm:py-10">
        ...
        <section className="w-full max-w-2xl overflow-hidden rounded-3xl border border-ink/10 bg-gradient-to-b from-cream to-surface shadow-sm">
          <div className="flex flex-col items-center gap-3 px-6 py-6 text-center sm:gap-5 sm:py-14">
            {published ? (
              <>
                {/* Post-race: the icon mark carries the branding and the name is
                    spelled out as the page's heading, so it reads the same in
                    both colour themes. */}
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src="/logo-mark.png" alt="" className="h-16 w-auto sm:h-28" />
                <h1 className="text-4xl font-black leading-tight text-ink sm:text-6xl">{t('publishedTitle')}</h1>

                {/* The two ways back into the race: this year's results, and
                    every year's. */}
                ...
                {/* Next year isn't dated yet — say so where the date used to be. */}
                <span className="mt-2 rounded-lg border-2 border-bike-dark/50 bg-bike/15 px-4 py-1 text-base font-extrabold tracking-[0.2em] text-bike-dark sm:px-5 sm:py-1.5 sm:text-xl">
                  {t('stayTuned')}
                </span>
                <p className="text-base font-medium text-ink sm:text-xl">{t('dateToBePublished')}</p>
              </>
            ) : (
              <>
