// Hall of Fame computations. These are pure functions over a list of results so
// they work with either the seeded static sheet data or the live, admin-edited
...
export function familyLabel(family: Family, locale: string): string {
  return locale === 'he' ? FAMILY_HE[family] ?? family : family;
}
...
export function buckets(results: HofResult[]): Bucket[] {
  const seen = new Set<string>();
  const out: Bucket[] = [];
  for (const family of FAMILY_ORDER) {
    for (const isTeam of [false, true]) {
      const key = `${family}:${isTeam}`;
      if (results.some((r) => r.family === family && r.isTeam === isTeam) && !seen.has(key)) {
        seen.add(key);
...
export function courseRecords(results: HofResult[]): RecordEntry[] {
  return buckets(results)
    .map(({ family, isTeam }) => {
      const win = fastest(results.filter((r) => r.family === family && r.isTeam === isTeam));
...
export type AnnotatedResult = HofResult & { place: number };
...
export function medalTable(results: HofResult[], includeGroups = false): Medalist[] {
  ...
  // A podium is one actual race: a specific (year, category, solo/team). Grouping
  // by the exact categoryHe — not the coarse family — keeps separate races (e.g.
  // "family" vs "team", or kids grades a-c vs d-f) as their own podiums.
  const podiums = new Map<string, HofResult[]>();
  for (const r of results) {
    if (r.isTeam && !includeGroups) continue;
    const key = `${r.year} ${r.categoryHe} ${r.isTeam}`;
    (podiums.get(key) ?? podiums.set(key, []).get(key)!).push(r);
  }
  for (const rows of podiums.values()) {
    rows.sort((a, b) => a.seconds - b.seconds);
    ...
    for (const r of rows) {
      ...
      if (place < 0) continue;
      // Dedupe: a roster that lists the same person twice (a sheet typo) still
      // earns them a single medal for that race.
      const names = [...new Set(r.isTeam ? r.members ?? [] : [r.name])];
