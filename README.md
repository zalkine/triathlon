...
8. As competitors reach each downstream station, that timekeeper searches for the name/team and taps to stamp the time. Finish-line stamps are the ones that determine the overall time.
   - A stamp is confirmed **on the competitor's own card** — the time and an **Undo** appear where the timekeeper just tapped, and nothing is ever drawn over the list, so a confirmation can't hide the next competitor to arrive or the button that stamps them.
   - The swim and bike stations drop a competitor off the list once stamped, but keep them in place — greyed out, with their time and Undo — while the undo window lasts (`UNDO_WINDOW_MS` in `src/components/StampStationView.tsx`, matching the server's limit in `undoEntryTime`), then let them go. The **finish line** keeps everyone on screen for good instead: a stamped competitor stays in place with their recorded time, so the list never reshuffles under the timekeeper's thumb and they can see who they've already taken.
