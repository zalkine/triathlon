...
8. As competitors reach each downstream station, that timekeeper searches for the name/team and taps to stamp the time. Finish-line stamps are the ones that determine the overall time.
   ...
   - Finish-line cards are colour-coded per category (`CATEGORY_COLOR_CLASS` in `src/lib/constants.ts`, tints in `globals.css`), and a chip row filters the list to a single category — one volunteer per race. The chosen filter is remembered per device, and a banner says how many waiting competitors it's hiding.
